...
def silhouette_score_block(X, labels, metric='euclidean', n_jobs=1, sample_size=None,
                           random_state=None, **kwds):
...
if __name__ == '__main__':
    ...
    np.random.seed(0)
    X = np.random.randint(low = 0, high = 1, size = (dim1, dim2))

    y = np.array([random.randint(1,10) for _ in range(dim1)])

    
    t0 = time.time()
    s = silhouette_score(X, y, metric='hamming', n_jobs = 4)

    t = time.time() - t0
    print('Scikit silhouette (%fs): %f' % (t, s))
    t0 = time.time()
    s = silhouette_score_block(X, y, metric = 'hamming')
    t = time.time() - t0
    print('Block silhouette (%fs): %f' % (t, s))
    t0 = time.time()
    s = silhouette_score_block(X, y, metric = 'hamming', n_jobs=4)
    t = time.time() - t0
